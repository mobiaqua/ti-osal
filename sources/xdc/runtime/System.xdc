...
module System {
     ...
     *  Write formated output to a character buffer
     *
     *  This function is identical to `{@link #sprintf}` except that atmost
     *  `n` characters is copied to the specified character buffer `buf` 
     ...
     *  at the end of the characters actually written into the buffer.
     *
     *  @param(buf) a character output buffer
     ...
     *
     *  @a(returns)
     *  `snprintf` returns the number of characters returns the number of 
     *  characters that would have been written had `n` been sufficiently 
     *  large, not counting the terminating '\0' character.
     */
